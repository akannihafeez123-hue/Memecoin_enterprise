...
// ╔══════════════════════════════════════════════════════════════════════════╗
// ║          MEMECOIN EMPIRE — ULTIMATE ENGINE v6.0                        ║
...
// ║           Real Market Data (live price/vol/liquidity/holders) +        ║
// ║           Fully Autonomous — scanner runs itself, Telegram pushes      ║
// ║           alerts automatically. Commands: /top10 /thresholds /status   ║
...
async function fetchTokenOnChainData(mint, dexHint = null) {
  ...
  // ── On-chain: recent tx signatures + velocity ──────────────────────────
  let recentTxCount = 0;
  let sigTimestamps = [];
  try {
  ...
  // ── DEX data: use cache hit → fresh fetch → safe defaults ─────────────
  // dexHint is passed in when the token was auto-discovered (already fetched).
  // For watchlist/manual tokens, fetch fresh from DexScreener.
  let dex = dexHint;
  if (!dex) {
    dex = await fetchDexPairData(mint);
  }

  // ── Holder data: Birdeye if key present, else estimate ─────────────────
  // Run concurrently with the above where possible.
  const holderData = await fetchHolderData(mint);

  // ── Resolve each field: real data first, then tx-based estimate ─────────

  // Price + market data from DEX (real)
  ...
  // LP depth = DEX liquidity USD (real pool depth, not simulated)
  const lpDepth = dex?.lpDepth ?? (supply > 0 ? 10_000 : 0);

  // LP growth rate = volume acceleration proxy from DexScreener h1/h24 (real)
  const lpGrowthRate = dex?.lpGrowthRate ?? 0.05;

  // Volume spike = 1h vol vs average hourly vol (real ratio, not random)
  // Falls back to tx-count heuristic if no DEX data
  const volumeSpike = dex?.volumeSpike != null
    ? dex.volumeSpike
    : recentTxCount > 50 ? 2.0 : 1.0;

  // Holder count: Birdeye first, DEX tx proxy second, on-chain tx estimate last
  const holderCount = holderData.holderCount > 0
    ? holderData.holderCount
    : Math.max(10, Math.floor(recentTxCount * 2.8));

  // Whale concentration: Birdeye top10% first, safe default second
  // 0.3 = conservative (30% held by top wallets) if we have no data
  const largeWalletRatio = holderData.largeWalletRatio > 0
    ? holderData.largeWalletRatio
    : 0.3;

  // Holder growth rate: buyer acceleration from DEX, or modest estimate
  ...
  return {
    ...
    // Pass through raw DEX fields for diagnostics
...
// ════════════════════════════════════════════════════════════════════════════
// SECTION 5A — REAL DEX DATA ENRICHMENT (v5.0)
// Replaces ALL Math.random() stubs with live DexScreener + Birdeye data.
// fetchDexPairData   — real price, volume, liquidity, volumeSpike
// fetchHolderData    — real holder count + top-wallet concentration
// _dexCache          — populated by discovery so tokens already-fetched
//                      don't pay a second HTTP round-trip in scoreMint
// ════════════════════════════════════════════════════════════════════════════

// Per-cycle cache: discovery pre-fetches DEX data for discovered tokens.
// scoreMint checks here first to avoid double-fetching.
...
async function fetchDexPairData(mint) {
  try {
    ...
    // Use the pair with deepest liquidity — most accurate price source
    ...
    // Volume spike = current 1h vs average 1h over 24h window
    const avgHourlyVol = vol24h / 24;
    const volumeSpike  = avgHourlyVol > 0 ? vol1h / avgHourlyVol : 1;

    // LP growth proxy: is volume accelerating? (6h annualised vs 24h)
    const lpGrowthRate = vol24h > 0
      ? clamp((((vol6h * 4) / vol24h) - 1), -1, 2)
      : 0;

    // Holder growth proxy from tx acceleration
    ...
    // Ratio of buyers this hour vs average — compressed into 0–1 scale
    ...
    return {
      ...
      txns1h, txns24h, txBuys1h, txSells1h,
...
async function fetchHolderData(mint) {
  // Without Birdeye key: estimate concentration from tx pattern
  if (!config.birdeyeKey) {
    return { holderCount: 0, largeWalletRatio: 0.3, source: "estimated" };
  }
  try {
    // Token security endpoint — top-holder concentration + unique wallets
    const res = await axios.get(
      `https://public-api.birdeye.so/defi/token_security?address=${mint}`,
      { headers: { "X-API-KEY": config.birdeyeKey }, timeout: 5000 }
    );
    const d = res.data?.data;
    if (!d) return { holderCount: 0, largeWalletRatio: 0.3, source: "birdeye_empty" };

    // top10HolderPercent: % of supply held by top 10 wallets (0–100)
    ...
    return { holderCount: 0, largeWalletRatio: 0.3, source: "error" };
  }
}
...
async function scoreMint(mint) {
  // Pull any pre-fetched DEX data from the discovery cache (avoids double HTTP call)
...
async function runScanCycle(mints = []) {
  ...
  // ── 2. Populate dex cache so scoreMint skips re-fetching discovered tokens ──
  _dexCache.clear();
  for (const d of discovered) {
    // Map discovered token fields to the shape fetchDexPairData returns
  ...
  // ── 3. Merge: auto-discovered + watchlist + manual seeds + provided ─────
  let wl = [];
...
bot.command("top10", async (ctx) => {
  try {
    ...
    if (!top.length) { await ctx.reply("No signals yet — scanner runs automatically every " + config.scanIntervalMinutes + " min."); return; }
    const text = top.map((s,i) => [
      ...
      `Wyckoff: ${s.wyckoff_phase ?? "?"} | Dir: ${dirLabel(s.direction)}`,
      `Mint: ${s.mint}`,
    ].join("\n")).join("\n\n");
...
// /dex <mint> — show raw live market data without full analysis
bot.command("dex", async (ctx) => {
  ...
  try {
    const [dex, holders] = await Promise.all([
      fetchDexPairData(mint),
      fetchHolderData(mint),
    ]);
    if (!dex) { await ctx.reply("No DEX pair found for this mint on Solana."); return; }
    await ctx.reply([
      `Live DEX Data`,
      `Mint: ${mint}`,
      ``,
      ...
      ``,
      `Volume 1h:  $${(dex.vol1h / 1000).toFixed(1)}k`,
      `Volume 24h: $${(dex.vol24h / 1000).toFixed(1)}k`,
      `Vol Spike:  ${dex.volumeSpike.toFixed(2)}x`,
      ``,
      `1h change:  ${dex.priceChange1h > 0 ? "+" : ""}${dex.priceChange1h.toFixed(1)}%`,
      `6h change:  ${dex.priceChange6h > 0 ? "+" : ""}${dex.priceChange6h.toFixed(1)}%`,
      `24h change: ${dex.priceChange24h > 0 ? "+" : ""}${dex.priceChange24h.toFixed(1)}%`,
      ``,
      `Buys 1h: ${dex.txBuys1h}  Sells 1h: ${dex.txSells1h}`,
      `Txns 24h: ${dex.txns24h}`,
      `LP Growth (proxy): ${(dex.lpGrowthRate * 100).toFixed(1)}%`,
      `DEX: ${dex.dexId ?? "?"}  Pair: ${dex.pairAddress?.slice(0,12) ?? "?"}...`,
      ``,
...
async function notifyAdmin(result) {
  const msg = [
    ...
    ``,
    `-- Scores --`,
    `Whale: ${result.whaleScore}  Smart: ${result.smartMoney}  Vol: ${result.volumeAnomaly}`,
    `Holders: ${result.holderVelocity}  Liq: ${result.liquidity}`,
    `Momentum: ${result.momentumScore}  Wyckoff: ${result.wyckoffScore}`,
    `HiddenFlow: ${result.hiddenFlow}  MultiTF: ${result.mtfScore}`,
    ``,
    ...
    `Data: DEX=${result.rawData?.onChain?.dexSource ?? "?"} Holders=${result.rawData?.onChain?.holderSource ?? "?"}`,
  ].join("\n");
...
async function main() {
  console.log("╔═══════════════════════════════════════════════════╗");
  console.log("║  MEMECOIN EMPIRE — ULTIMATE ENGINE v6.0           ║");
  console.log("║  Fully Autonomous Pre-Pump Detection              ║");
  console.log("║  Auto-scans DEX every " + String(config.scanIntervalMinutes).padEnd(3) + " min — pushes alerts   ║");
  console.log("║  Commands: /top10  /thresholds  /status           ║");
  console.log("╚═══════════════════════════════════════════════════╝");
